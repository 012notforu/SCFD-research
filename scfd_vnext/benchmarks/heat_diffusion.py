...
def synthetic_temperature(shape: Tuple[int, int], kind: str = "gradient") -> Array:
    h, w = shape
    yy, xx = np.meshgrid(np.linspace(0, 1, h), np.linspace(0, 1, w), indexing="ij")
    if kind == "gradient":
        pattern = xx
    elif kind == "hot_corner":
        pattern = np.exp(-((xx - 0.2) ** 2 + (yy - 0.2) ** 2) / 0.02)
    elif kind == "cool_spot":
        pattern = 1.0 - 0.8 * np.exp(-((xx - 0.8) ** 2 + (yy - 0.8) ** 2) / 0.02)
    else:
