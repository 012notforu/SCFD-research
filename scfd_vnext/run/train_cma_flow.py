...
def _vector_from_config(control: FlowCylinderControlConfig, params: FlowCylinderParams) -> np.ndarray:
    return np.array(
        [
            control.encode_gain,
...
def _config_from_vector(
    ...
    base_params: FlowCylinderParams,
    vector: np.ndarray,
) -> Tuple[FlowCylinderControlConfig, FlowCylinderParams]:
...
def _evaluate(
    ...
    energies: List[float] = []
    for seed in seeds:
        sim_params = replace(params, init_seed=int(seed), inflow=inflow)
        ...
        mses.append(float(last["wake_mse"]))
        drag_list.append(float(last["drag_proxy"]))
        energies.append(float(last["energy"]))
    mse_arr = np.array(mses, dtype=np.float32)
    drag_arr = np.array(drag_list, dtype=np.float32)
    energy_arr = np.array(energies, dtype=np.float32)
    metrics = {
        "mean_wake_mse": float(mse_arr.mean()),
        "std_wake_mse": float(mse_arr.std()),
        "mean_drag": float(drag_arr.mean()),
        "mean_energy": float(energy_arr.mean()),
    }
    score = -metrics["mean_wake_mse"]
    ...
    with path.open("a", newline="") as fh:
        ...
        writer.writerow(
            [
                ...
                candidate,
                score,
                metrics["mean_wake_mse"],
                metrics["std_wake_mse"],
                metrics["mean_drag"],
                metrics["mean_energy"],
                int(is_best),
            ]
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="CMA-style search for SCFD flow cylinder controller")
    ...
    parser.add_argument("--outdir", type=str, default="runs/flow_cylinder_cma")
    parser.add_argument("--early-stop-score", type=float, default=-1e-3, help="Stop when score >= threshold")
    parser.add_argument("--early-stop-count", type=int, default=5, help="Generations to sustain early-stop score")
...
def main() -> None:
    ...
    for gen in range(args.generations):
        ...
        print(
            f"[gen {gen:03d}] best_score={elites[0][0]:.6f} mean_wake_mse={elites[0][2]['mean_wake_mse']:.6f} global_best={-best_score:.6f} mse, sigma={sigma:.4f}",
            flush=True,
        )
    ...
    final_params = replace(final_params, inflow=inflow)
    simulator = FlowCylinderSimulator(final_params, FlowCylinderController(final_control, final_params.shape))
