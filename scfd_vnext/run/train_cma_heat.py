...
def _vector_from_config(control: HeatDiffusionControlConfig, params: HeatDiffusionParams) -> np.ndarray:
    return np.array(
        [
            control.encode_gain,
...
def _config_from_vector(
    ...
    vector: np.ndarray,
) -> Tuple[HeatDiffusionControlConfig, HeatDiffusionParams]:
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="CMA-style search for SCFD heat diffusion controller")
    ...
    parser.add_argument("--outdir", type=str, default="runs/heat_diffusion_cma")
...
def main() -> None:
    ...
    for gen in range(args.generations):
        ...
        print(
            f"[gen {gen:03d}] best_score={elites[0][0]:.5f} mean_mse={elites[0][2]['mean_mse']:.5f} global_best={-best_score:.5f} mse, sigma={sigma:.4f}",
            flush=True,
        )
    ...
    target = synthetic_temperature(final_params.shape, kind=args.target)
    simulator = HeatDiffusionSimulator(final_params, controller, target)
