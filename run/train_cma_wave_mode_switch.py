...
def _vector_from_config(control: WaveFieldControlConfig, params: WaveModeSwitchParams) -> np.ndarray:
    return np.array(
        [
            control.encode_gain,
...
def _config_from_vector(
    ...
    vector: np.ndarray,
) -> Tuple[WaveFieldControlConfig, WaveModeSwitchParams]:
    ...
    wave_speed = _clip(vec[6], 0.3, 2.5)
...
def _evaluate(
    ...
    initial_mse: List[float] = []
    switched_mse: List[float] = []
    energies: List[float] = []
    for seed in seeds:
        sim_params = replace(params, init_seed=int(seed))
        controller = WaveFieldController(control_cfg, sim_params.shape)
        simulator = WaveModeSwitchSimulator(sim_params, controller)
        history = simulator.run(steps=steps, record_interval=record_interval)
        metrics = history["metrics"]
        init_values = [m["mse"] for m in metrics if m["phase"] == "initial"]
        switch_values = [m["mse"] for m in metrics if m["phase"] == "switched"]
        energies.extend(float(m["energy"]) for m in metrics[-3:])
        if init_values:
            initial_mse.append(float(np.mean(init_values)))
        if switch_values:
            switched_mse.append(float(np.mean(switch_values)))
    init_arr = np.array(initial_mse or [1.0], dtype=np.float32)
    switch_arr = np.array(switched_mse or [1.0], dtype=np.float32)
    energy_arr = np.array(energies, dtype=np.float32) if energies else np.array([0.0], dtype=np.float32)
    metrics = {
        "mean_initial_mse": float(init_arr.mean()),
        "mean_switched_mse": float(switch_arr.mean()),
        "mean_energy": float(energy_arr.mean()),
    }
    score = -0.5 * (metrics["mean_initial_mse"] + metrics["mean_switched_mse"]) - 0.05 * metrics["mean_energy"]
    ...
    with path.open("a", newline="") as fh:
        ...
        writer.writerow(
            [
                ...
                candidate,
                score,
                metrics["mean_initial_mse"],
                metrics["mean_switched_mse"],
                metrics["mean_energy"],
                int(is_best),
            ]
        )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="CMA search for wave mode-switch benchmark")
    ...
    parser.add_argument("--switch-step", type=int, default=600)
    parser.add_argument("--initial-kind", type=str, default="focus", choices=["focus", "defocus", "waves"])
    parser.add_argument("--switch-kind", type=str, default="waves", choices=["focus", "defocus", "waves"])
    parser.add_argument("--outdir", type=str, default="runs/wave_mode_switch_cma")
...
def main() -> None:
    ...
    for gen in range(args.generations):
        ...
        print(
            f"[gen {gen:03d}] best_score={elites[0][0]:.6f} initial_mse={elites[0][2]['mean_initial_mse']:.6f} "
            f"switch_mse={elites[0][2]['mean_switched_mse']:.6f} global_best={-best_score:.6f} loss, sigma={sigma:.4f}",
            flush=True,
        )
    ...
    simulator = WaveModeSwitchSimulator(final_params, controller)
