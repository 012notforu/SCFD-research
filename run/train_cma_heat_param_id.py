...
def _config_from_vector(
    ...
    vector: np.ndarray,
) -> Tuple[HeatDiffusionControlConfig, HeatParameterIDParams]:
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="CMA search for heat parameter identification controller")
    ...
    parser.add_argument("--outdir", type=str, default="runs/heat_param_id_cma")
...
def main() -> None:
    ...
    for gen in range(args.generations):
        candidates: List[Tuple[float, np.ndarray, Dict[str, float], HeatDiffusionControlConfig, HeatParameterIDParams]] = []
        ...
        print(
            f"[gen {gen:03d}] score={elites[0][0]:.6f} mse={elites[0][2]['mean_mse']:.6f} alpha_rmse={elites[0][2]['mean_alpha_rmse']:.6f} best={best_score:.6f} sigma={sigma:.4f}",
            flush=True,
        )
    ...
    simulator = HeatParameterIDSimulator(final_params, controller)
