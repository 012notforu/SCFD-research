...
def _vector_from_config(control: FlowCylinderControlConfig, params: FlowRedundantParams) -> np.ndarray:
    return np.array(
        [
            control.encode_gain,
...
def _config_from_vector(
    ...
    vector: np.ndarray,
) -> Tuple[FlowCylinderControlConfig, FlowRedundantParams]:
    ...
    viscosity = _clip(vec[6], 0.01, 0.06)
    budget = _clip(vec[7], 0.1, 10.0)
...
def _evaluate(
    ...
    for seed in seeds:
        ...
        actuator_rms.append(float(last["actuator_rms"]))
    ...
    rms_arr = np.array(actuator_rms, dtype=np.float32)
    metrics = {
        ...
        "mean_actuator_rms": float(rms_arr.mean()),
    }
    score = (
        metrics["mean_throughput"]
        - 0.1 * metrics["mean_energy"]
        - 0.05 * metrics["mean_budget"]
        - 0.05 * metrics["mean_actuator_rms"]
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="CMA search for redundant body-force flow benchmark")
    ...
    parser.add_argument("--outdir", type=str, default="runs/flow_redundant_cma")
...
def main() -> None:
    ...
    for gen in range(args.generations):
        ...
        print(
            f"[gen {gen:03d}] best_score={elites[0][0]:.6f} throughput={elites[0][2]['mean_throughput']:.6f} "
            f"energy={elites[0][2]['mean_energy']:.4f} budget={elites[0][2]['mean_budget']:.3f} "
            f"global_best={best_score:.6f} score, sigma={sigma:.4f}",
            flush=True,
        )
    ...
    simulator = FlowRedundantSimulator(final_params, controller)
