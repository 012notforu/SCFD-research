...
def _config_from_vector(
    ...
    vector: np.ndarray,
) -> Tuple[HeatDiffusionControlConfig, HeatDiffusionRoutingParams]:
    ...
    collision_radius = _clip(vec[7], 0.02, 0.2)
...
def _metadata_from_params(
    ...
    return {
        ...
        "initial_centers": [list(center) for center in params.initial_centers],
        "target_centers": [list(center) for center in params.target_centers],
        "blob_sigma": float(params.blob_sigma),
        "collision_radius": float(params.collision_radius),
    }
...
def _evaluate(
    ...
    for seed in seeds:
        ...
        simulator = HeatDiffusionRoutingSimulator(sim_params, controller)
        history = simulator.run(steps=steps, record_interval=record_interval)
        last = history["metrics"][-1]
        mses.append(float(last["mse"]))
        collisions.append(float(last["collision_penalty"]))
    mse_arr = np.array(mses, dtype=np.float32)
    collision_arr = np.array(collisions, dtype=np.float32)
    metrics = {
        "mean_mse": float(mse_arr.mean()),
        "std_mse": float(mse_arr.std()),
        "mean_collision": float(collision_arr.mean()),
    }
    score = -metrics["mean_mse"] - 0.5 * metrics["mean_collision"]
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="CMA search for heat routing controller")
    ...
    parser.add_argument("--outdir", type=str, default="runs/heat_routing_cma")
...
def main() -> None:
    ...
    for gen in range(args.generations):
        candidates: List[Tuple[float, np.ndarray, Dict[str, float], HeatDiffusionControlConfig, HeatDiffusionRoutingParams]] = []
        ...
        print(
            f"[gen {gen:03d}] score={elites[0][0]:.6f} mse={elites[0][2]['mean_mse']:.6f} collision={elites[0][2]['mean_collision']:.6f} best={best_score:.6f} sigma={sigma:.4f}",
            flush=True,
        )
    ...
    simulator = HeatDiffusionRoutingSimulator(final_params, controller)
