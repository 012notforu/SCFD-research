...
def _config_from_vector(
    ...
    vector: np.ndarray,
) -> Tuple[HeatDiffusionControlConfig, HeatFrontParams]:
...
def _metadata_from_params(
    ...
    return {
        ...
        "front_radius": float(params.front_radius),
...
def _evaluate(
    ...
    for seed in seeds:
        ...
        simulator = HeatFrontTrackingSimulator(sim_params, controller)
        history = simulator.run(steps=steps, record_interval=record_interval)
        last = history["metrics"][-1]
        mses.append(float(last["mse"]))
        curvature.append(float(last["curvature_proxy"]))
    mse_arr = np.array(mses, dtype=np.float32)
    cur_arr = np.array(curvature, dtype=np.float32)
    metrics = {
        "mean_mse": float(mse_arr.mean()),
        "std_mse": float(mse_arr.std()),
        "mean_curvature": float(cur_arr.mean()),
    }
    score = -metrics["mean_mse"] - 0.2 * metrics["mean_curvature"]
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="CMA search for heat front tracking controller")
    ...
    parser.add_argument("--outdir", type=str, default="runs/heat_front_cma")
...
def main() -> None:
    ...
    for gen in range(args.generations):
        candidates: List[Tuple[float, np.ndarray, Dict[str, float], HeatDiffusionControlConfig, HeatFrontParams]] = []
        ...
        print(
            f"[gen {gen:03d}] score={elites[0][0]:.6f} mse={elites[0][2]['mean_mse']:.6f} curvature={elites[0][2]['mean_curvature']:.6f} best={best_score:.6f} sigma={sigma:.4f}",
            flush=True,
        )
    ...
    simulator = HeatFrontTrackingSimulator(final_params, controller)
